$(document).ready(function () {
    ...
    $(this).autocomplete({
      source: function (request, response) {
        $.ajax({
          ...
          url:
            _baseURL +
            "dashboard_doctor/prescription/prescription/readMedicine",
  ...
  //#------------------------------------
  //add row
  $("body").on("click", ".YogaAddBtn", function () {
    var itemData = $(this).parent().parent().parent();
    $("#yoga").append("<tr>" + itemData.html() + "</tr>");
    $("#yoga tr:last-child").find(":input").val("");
  });
  //remove row
  $("body").on("click", ".YogaRemoveBtn", function () {
    $(this).parent().parent().parent().remove();
  });
  //#------------------------------------
  //   STARTS OF price
  //#------------------------------------
  //add row
  $("body").on("click", ".priceAddBtn", function () {
    var itemData = $(this).parent().parent().parent();
    $("#price").append("<tr>" + itemData.html() + "</tr>");
    $("#price tr:last-child").find(":input").val("");
  });
  //remove row
  $("body").on("click", ".priceRemoveBtn", function () {
    $(this).parent().parent().parent().remove();
  });
  ...
  $(window).on("load", function () {
    ...
    if (patient_id.length > 0) patientInfo(patient_id);
  });
